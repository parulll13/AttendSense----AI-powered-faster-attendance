...
def voice_attendance_dialog(selected_subject_id):
    ...
    if st.button("Analyze audio", width='stretch', type='primary'):
        with st.spinner("Processing the audio data.."):
            ...
            # Store the attendance results and attendance logs in session state
            # so they remain available after Streamlit reruns the script
            st.session_state.voice_attendance_results = (pd.DataFrame(results), attendance_to_log)

    # Check whether voice attendance results are available
